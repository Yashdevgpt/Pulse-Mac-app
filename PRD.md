...
- **AI Keys panel:** Top section above the user list lets the operator paste a Gemini API key and an OpenRouter API key. Inputs are password-masked with a per-field show/hide toggle, a "Saved (N chars, ends XXXX)" hint underneath, and Save / Clear buttons. Save persists keys to a private on-disk file via the local server (`POST /api/ai-keys` → `ai-keys.json` beside the runtime `.env`) so they survive app restarts; `localStorage` (`pulse:ai-keys:v1`) remains the in-session cache. Keys are never sent to Firestore or any cloud service. The client attaches them as `x-gemini-key` / `x-openrouter-key` headers on every Brain API call, and the server consumes them per-request.
...
- **AI:** Google Gemini (`gemini-2.5-flash` for chat, `gemini-embedding-001` at 768 dimensions for embeddings) with an OpenRouter fallback chain for chat completions (`deepseek/deepseek-chat`, then `openai/gpt-oss-120b`; override via `OPENROUTER_MODELS`).
  ...
  - **Provider fallback:** `runChatWithFallback(req, res, prompt, geminiConfig)` tries Gemini first; on errors classified by `isGeminiFallbackError(error)` (HTTP 401 / 403 / 429, or messages containing `rate limit`, `quota`, `RESOURCE_EXHAUSTED`, `exceeded`, `api key not valid`, `invalid api key`) it transparently retries against OpenRouter's OpenAI-compatible endpoint (`https://openrouter.ai/api/v1/chat/completions`), walking the `OPENROUTER_MODELS` chain in order (default `deepseek/deepseek-chat`, then `openai/gpt-oss-120b`). The provider and model used are exposed via the `X-Provider-Used: gemini|openrouter` and `X-Openrouter-Model` response headers and the `provider` field in `/api/brain/chat`. The client throttles a single "Switched to OpenRouter" toast (naming the model used) per calendar day via `consumeFallbackToastSlot()` (`pulse:ai-keys:fallback-toast-date`).
...
- **Configuration:**
  - **AI keys → on-disk file + in-app cache.** Edited via Admin → AI Keys. The local server persists them in `ai-keys.json` beside the runtime `.env` (`GET`/`POST /api/ai-keys`, Firebase-token-gated, file mode 0600); `localStorage` (`pulse:ai-keys:v1`) is the synchronous in-session cache, restored from disk at sign-in. No env var path for the keys themselves. `clearAiKeysEverywhere()` and a `pulse:ai-keys:changed` `CustomEvent` keep the panel in sync after edits.
...
- **Security:**
  ...
  - AI keys are persisted only to `ai-keys.json` beside the runtime `.env` on this Mac (file mode 0600, written by the local server). Both `/api/ai-keys` endpoints require the bootstrap admin account — a merely-authenticated Firebase token is not enough, since signup is open and the key file is machine-global. Brain calls still carry keys per-request in headers; key values are never logged and never persisted to Firestore or any cloud service.
...
### 2026-06-10 - Persistent AI Keys, Memory Batching, Fallback Chain, and Retrieval Quality
- **Approval:** User reported four issues and approved the fixes after discussion: (1) AI keys had to be re-entered after every app restart; (2) "Rebuild Memory" failed once the account held more than 80 sources, which also blocked indexing large Brain cards; (3) Brain answers were low quality; (4) the `gpt-5-nano` fallback was too weak. User chose `deepseek/deepseek-chat` plus a second cheap OpenRouter model, approved dropping the lexical source filter, and approved auto-indexing Fleet saves. Design revamp explicitly deferred.
- **Root causes:**
  - The Electron shell loads `http://localhost:<random free port>`, so the `localStorage` origin changes every launch and abandons stored AI keys (and the Firebase session) on restart. The user declined port pinning, so keys moved to an on-disk file instead.
  - `/api/brain/index` rejects >80 sources per request and the Express JSON body cap was 2 MB, while Rebuild Memory sent every source in one request.
  - A lexical post-filter (`filterRelevantPrivateSources`) dropped vector-search hits whose text did not literally contain a question word, starving the model of grounding and triggering the strict refusal sentence.
  - DSPs, logs, and tags were only indexed into vector memory on a manual Rebuild Memory click (saves never indexed; only deletes were propagated), so answers about Fleet data went stale.
- **Changes:**
  - **AI key persistence (`server.mjs`, `src/lib/aiKeys.ts`, `src/pages/Admin.tsx`, `src/components/AuthWrapper.tsx`):** New `GET`/`POST /api/ai-keys` endpoints (Firebase-token-gated, rate-limited, key format validated) backed by `ai-keys.json` written with mode 0600 beside the runtime `.env` (production: `~/Library/Application Support/Pulse/`; dev: `.ai-keys.local.json`, gitignored). `saveAiKeysEverywhere()` / `clearAiKeysEverywhere()` write both stores; `syncAiKeysFromServer()` runs at sign-in, restores keys into the `localStorage` cache, and migrates legacy localStorage-only keys up to the file. The Admin panel refreshes its inputs on `pulse:ai-keys:changed`.
  - **Memory indexing batching (`src/lib/brainApi.ts`, `src/pages/Brain.tsx`, `server.mjs`):** `indexBrainSources()` now splits requests into batches bounded by 40 sources AND ~4 MB of serialized JSON, reports progress (`Indexing memory… N/M sources` toast during Rebuild), and sums per-batch results. Express JSON body limit raised from 2 MB to 25 MB (server remains bound to `127.0.0.1`).
  - **OpenRouter fallback chain (`server.mjs`, `src/lib/brainApi.ts`):** Replaced the hardcoded `openai/gpt-5-nano` with an ordered chain — `deepseek/deepseek-chat`, then `openai/gpt-oss-120b` — overridable via `OPENROUTER_MODELS` (comma-separated). Models are tried in order on failure; a missing-key 412 aborts immediately. New `X-Openrouter-Model` response header; the once-per-day fallback toast now names the model actually used. One OpenRouter API key covers the whole chain.
  - **Retrieval quality (`server.mjs`):** Removed the lexical word-match post-filter and its helpers (`SEARCH_STOP_WORDS`, `sourceTypeQueryHints`, `getSearchTerms`, `expandSearchTerms`, `sourceHasSearchTerm`, `sourceMatchesType`, `getNonTypeQuestionTerms`, `isRelevantPrivateSource`, `filterRelevantPrivateSources`, `getQuestionSourceTypes`). Private sources now flow from the pgvector similarity search (which already enforces `min_similarity`), recent-intent sources, and client passages through dedupe only. Source-type inference still narrows the vector search.
  - **Fleet auto-indexing (`src/lib/brainApi.ts` + call sites):** New fire-and-forget helpers `autoIndexDspRecord`, `autoIndexFleetLog` (also refreshes the parent DSP record, whose content embeds log counts and the latest-log summary), and `autoIndexTagRecord`. Wired into: Bridge DSP create and log submit; Fleet star toggle and rename; Logbook edit, file attach, and delete (DSP refresh); Watchtower tag create and task resolve. Helpers silently no-op without a Gemini key and log-warn on failure — Rebuild Memory remains the recovery path.
- **Operational notes:**
  - Firebase sign-in is still required after each app restart (session storage is origin-bound to the random port); only the AI keys persist. Fixing session persistence needs a stable origin (deferred).
  - Verified against the live OpenRouter catalog on 2026-06-10: `deepseek/deepseek-chat` ($0.20/M prompt) and `openai/gpt-oss-120b` ($0.039/M prompt) both available; one key covers both.
- **Logs:**
  - `node --check server.mjs` passed.
  - `npm run check` (typecheck + production build) passed with the existing large-chunk warning only.
- **Post-review hardening (same session):** A follow-up security and correctness review of the diff confirmed and fixed five follow-ups:
  - `/api/ai-keys` now requires the bootstrap admin account (`requireKeyStoreOwner`, HTTP 403 otherwise). Identity alone was insufficient: Firebase signup is open and the key file is machine-global, so any locally minted token could previously read or replace the owner's keys.
  - The 25 MB JSON body parser is scoped to `/api/brain/index` only; every other route keeps the original 2 MB cap.
  - `saveAiKeysEverywhere()` / `clearAiKeysEverywhere()` persist to disk first and only update the `localStorage` cache on success, so a rejected write can no longer leave the session and the disk store divergent.
  - `formatSources` prompt cap raised from 10 to 24 sources so recent-intent chunks, vector hits, and client passages all fit now that the lexical post-filter no longer prunes the combined list.
  - In production without `PULSE_ENV_PATH`, the key file falls back to the directory of the `.env` actually loaded (`__dirname`) instead of `process.cwd()`. Stale `gpt-5-nano` hint in the Admin panel and stale PRD/README/.env.example wording corrected.
  - Known accepted limitation: concurrent fire-and-forget auto-index calls for the same record can briefly persist stale chunks (last-write-loses); the design tolerates this and Rebuild Memory remains the recovery path.
